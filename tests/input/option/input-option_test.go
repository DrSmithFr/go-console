...
func TestConstructor(t *testing.T) {
	opt1 := option.NewInputOption("foo", option.OPTIONAL)
	assert.Equal(t, "foo", opt1.GetName())

	opt2 := option.NewInputOption("--foo", option.OPTIONAL)
...
func TestArrayModeWithoutValue(t *testing.T) {
	assert.Panics(t, func() {
		option.
			NewInputOption("foo", option.IS_ARRAY).
			SetShortcut("f")
	})
}
...
func TestInvalidModes(t *testing.T) {
	assert.Panics(t, func() {
		option.NewInputOption("foo", -1)
	})
}

func TestEmptyNameIsInvalid(t *testing.T) {
	assert.Panics(t, func() {
		option.NewInputOption("", option.NONE)
	})
}
...
func TestIsArray(t *testing.T)  {
	opt1 := option.NewInputOption("foo", option.OPTIONAL | option.IS_ARRAY)
...
func TestGetDescription(t *testing.T)  {
	opt1 := option.
		NewInputOption("foo", option.OPTIONAL | option.IS_ARRAY).
...
func TestSetDefaultOnArray(t *testing.T) {
	...
	assert.Panics(t, func() {
		option.
			NewInputOption("foo", option.IS_ARRAY | option.REQUIRED).
			SetDefault("default")
	})
}
